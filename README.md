...
**3-2. Supabase에서 Clerk를 서드파티 인증 제공자로 설정**
...
4. **"Add provider"** 클릭 → **"Clerk"** 선택
5. **"Clerk domain"** 필드에 3-1에서 복사한 도메인 입력:
   ```
...
- `auth.jwt()->>'sub'`를 통해 Clerk 사용자 ID에 접근할 수 있습니다
...
- RLS 정책이 자동으로 활성화되어 사용자별 데이터 보호

**5-2. 예제 스키마 적용 (tasks 테이블, 선택사항)**

작업 관리 예제 페이지(`/tasks-example`)를 사용하려면:

1. Supabase Dashboard → **SQL Editor** 메뉴
2. **"New query"** 클릭
3. `supabase/migrations/setup_tasks_example.sql` 파일 내용을 복사하여 붙여넣기
4. **"Run"** 클릭하여 실행

**생성되는 테이블:**
- `tasks`: 작업 관리 예제용 테이블 (Clerk 공식 문서 기반)
...
**테스트 페이지:**
- `/instruments-example`: Supabase 공식 문서 기반 표준 예제 (Supabase 자체 인증)
- `/tasks-example`: Clerk 통합 작업 관리 예제 (Clerk 인증)
...
**로컬라이제이션 파일:**
- `lib/clerk/localization.ts`: 한국어 로컬라이제이션 설정
- 기본 `koKR` 사용 (권장)
- 필요시 커스텀 메시지 추가 가능

**자세한 내용:**
- [Clerk 한국어 로컬라이제이션 가이드](./README-CLERK-LOCALIZATION.md)
- [Clerk 공식 로컬라이제이션 문서](https://clerk.com/docs/guides/customizing-clerk/localization)

**커스터마이징 예제:**
```tsx
// lib/clerk/localization.ts에서 커스텀 메시지 추가
...
Clerk와 Supabase 네이티브 통합에서는 `auth.jwt()->>'sub'`를 통해 Clerk 사용자 ID를 추출합니다. 이 값은 Clerk 세션 토큰의 `sub` 클레임에서 가져옵니다.

**추가 테이블 생성 시 RLS 정책 설정 예제:**

```sql
-- 테이블 생성 (Clerk user_id를 TEXT로 저장)
...
**참고:**
- `tasks` 테이블 예제는 `supabase/migrations/setup_tasks_example.sql` 파일을 참고하세요
- Clerk 공식 문서의 모범 사례를 따릅니다: [Clerk Supabase 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)
...
│   ├── instruments-example/  # Supabase 표준 예제 페이지
│   ├── tasks-example/        # Clerk 통합 예제 페이지
...
│   │   ├── server-standard.ts  # 표준 서버 클라이언트 (Supabase 인증)
│   │   ├── client-standard.ts  # 표준 브라우저 클라이언트 (Supabase 인증)
│   │   ├── clerk-client.ts     # Clerk 통합 클라이언트 (Client Component)
│   │   ├── server.ts           # Clerk 통합 서버 클라이언트
│   │   ├── service-role.ts     # 관리자용 (RLS 우회)
│   │   ├── client.ts           # 하위 호환성용
