...
- [x] 네이버 지도 연동 (MVP 2.2)
  ...
  - 실행 후 동작이상
    - [x] `getMarkerIcon` 함수 타입 문제 수정 - 반환 타입을 any로 변경하고 null 체크 추가
    - [x] `currentTours` 초기화 문제 수정 - useEffect로 initialTours 동기화
    - [x] 마커 생성 시점 문제 수정 - 조건 확인 강화 및 디버깅 로그 추가
    - [x] 디버깅 로그 추가 - 마커 생성 과정 추적 및 에러 메시지 개선
    - [x] `isMapReady` 상태 추가 - 지도 초기화 완료 감지 및 마커 생성 트리거
    - [x] 좌표 변환 로직 개선 - WGS84/KATEC 형식 자동 감지 및 변환
- [x] 페이지네이션
  - [x] 무한 스크롤 구현
    - [x] Intersection Observer 사용 (`hooks/use-intersection-observer.ts`)
    - [x] 하단 로딩 인디케이터
    - [x] 페이지당 20개 항목 (`lib/constants/pagination.ts`)
  - [ ] ~~또는 페이지 번호 선택 방식~~ (무한 스크롤 채택)
- [x] 최종 통합 및 스타일링
  - [x] 모든 기능 통합 테스트
  - [x] 반응형 디자인 확인 (모바일/태블릿/데스크톱)
  - [x] 로딩 상태 개선 (초기 로딩 vs 추가 로딩 분리)
  - [x] 에러 처리 개선 (재시도 버튼 추가)
  - 실행 후 동작이상
    - [x] API 타임아웃 증가 (10초 → 30초)
    - [x] 이미지 로딩 최적화 (`loading="lazy"`, `unoptimized` 옵션)
    - [x] useEffect 의존성 문제 수정 (중복 API 호출 방지)
    - [x] AbortController로 이전 요청 취소 처리
    - [x] 렌더링 중 상태 업데이트 문제 수정 (`onToursChange`를 `useEffect`로 분리)
